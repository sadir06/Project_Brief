...
- [Key Technical Contributions](#key-technical-contributions)
	...
	- [2. Verification](#2-verification) 
	  - [2.1 F1 Starting Lights Implementation and Testing](#2.1-f1-starting-lights-implementation-and-testing)
		- [2.2 Reference Program](#2.2-reference-program)
		- [2.3 Full RV32I Design Testing](#2.3-full-rv32i-design-testing)
...
- [Appendix: Test Results Summary](#appendix-test-results-summary)
...
        vbdHex(3, top->a0 & 0xF);
```
This enabled us to notice that the assembly code was not incrementing with the pattern 2n+1 as it was supposed to since it was being done in two operations. Therefore, we added a temporary register that would then be used to change the value of a0:
``` asm
increment_light:
...
Additionally, instead of changing the name of the distribution on the testbench file each time, I found that it was more elegant to write the shell script so that it took the distribution name as a target as well. For example, to run **noisy.mem** you would do
``` bash
